...
/*
 * RootHide bootstrap support
 *
 * Dopamine 3 keeps its modern exploit and offset code in the main classes,
 * while this category supplies the RootHide storage contract.  The category
 * is in this file so the normal Xcode target picks it up without replacing
 * the modern bootstrapper implementation wholesale.
 */

static uint64_t rootHideJbrandNew(void)
{
    uint64_t value = ((uint64_t)arc4random()) | ((uint64_t)arc4random() << 32);
    uint8_t check = (uint8_t)(value >> 8 ^ value >> 16 ^ value >> 24 ^
                              value >> 32 ^ value >> 40 ^ value >> 48 ^ value >> 56);
    return (value & ~0xffULL) | check;
}

static BOOL rootHideIsJbrandValue(uint64_t value)
{
    uint8_t check = (uint8_t)(value >> 8 ^ value >> 16 ^ value >> 24 ^
                              value >> 32 ^ value >> 40 ^ value >> 48 ^ value >> 56);
...
static uint64_t rootHideJbrandFromName(NSString *name)
{
    if (name.length != 24 || ![name hasPrefix:@".jbroot-"]) return 0;
    const char *suffix = name.UTF8String + 8;
    char *end = NULL;
    uint64_t value = strtoull(suffix, &end, 16);
    if (!end || *end != '\0' || !rootHideIsJbrandValue(value)) return 0;
    return value;
}

static NSString *rootHideFindJbroot(void)
{
    NSArray<NSString *> *items = [[NSFileManager defaultManager] contentsOfDirectoryAtPath:@"/var/containers/Bundle/Application" error:nil];
    for (NSString *item in items) {
        if (rootHideJbrandFromName(item) != 0) {
            return [@"/var/containers/Bundle/Application" stringByAppendingPathComponent:item];
        }
    }
    return nil;
}

static NSString *rootHideJbrootPath(NSString *path)
{
    NSString *root = rootHideFindJbroot();
    if (!root) return nil;
    if (!path || ![path hasPrefix:@"/"]) return path;
    return [root stringByAppendingPathComponent:path];
}

static NSError *rootHideError(NSInteger code, NSString *message)
{
    return [NSError errorWithDomain:bootstrapErrorDomain code:code userInfo:@{NSLocalizedDescriptionKey: message}];
}
...
- (void)prepareBootstrapWithCompletion:(void (^)(NSError *))completion
{
    ...
    // RootHide owns the randomized jbroot and deliberately does not expose
    // the rootless /var/jb alias.
    NSDictionary *varJbAttributes = [fm attributesOfItemAtPath:@"/var/jb" error:nil];
    if (varJbAttributes) {
        if (![fm removeItemAtPath:@"/var/jb" error:&error]) {
            completion(rootHideError(BootstrapErrorCodeFailedReplacing, [NSString stringWithFormat:@"Failed to remove /var/jb: %@", error]));
            return;
        }
    }

    NSString *jbroot = rootHideFindJbroot();
    if (!jbroot) {
        uint64_t jbrand = rootHideJbrandNew();
        jbroot = [NSString stringWithFormat:@"/var/containers/Bundle/Application/.jbroot-%016llX", jbrand];
        if (mkdir(jbroot.fileSystemRepresentation, 0755) != 0 && errno != EEXIST) {
            completion(rootHideError(BootstrapErrorCodeFailedExtracting, [NSString stringWithFormat:@"Failed to create jbroot: %s", strerror(errno)]));
            return;
        }
        chown(jbroot.fileSystemRepresentation, 0, 0);

        NSString *bootstrapName = [NSString stringWithFormat:@"bootstrap_%@.tar.zst", @"1900"];
        NSString *bootstrapPath = [NSBundle.mainBundle.bundlePath stringByAppendingPathComponent:bootstrapName];
        if (![[NSFileManager defaultManager] fileExistsAtPath:bootstrapPath]) {
            completion(rootHideError(BootstrapErrorCodeFailedExtracting, [NSString stringWithFormat:@"Missing bundled RootHide bootstrap: %@", bootstrapName]));
            return;
        }

        NSString *tarPath = [NSTemporaryDirectory() stringByAppendingPathComponent:@"roothide-bootstrap.tar"];
        [fm removeItemAtPath:tarPath error:nil];
        error = [self decompressZstd:bootstrapPath toTar:tarPath];
        if (error) {
            completion(error);
            return;
        }
        error = [self extractTar:tarPath toPath:jbroot];
        [fm removeItemAtPath:tarPath error:nil];
        if (error) {
            completion(error);
            return;
        ...
        uint64_t jbrand = rootHideJbrandFromName(jbroot.lastPathComponent);
        if (!jbrand) {
            completion(rootHideError(BootstrapErrorCodeFailedExtracting, @"Invalid RootHide jbroot name"));
            return;
        }
    ...
    NSString *secondary = [NSString stringWithFormat:@"/var/mobile/Containers/Shared/AppGroup/.jbroot-%016llX", gSystemInfo.jailbreakInfo.jbrand];
    if (mkdir(secondary.fileSystemRepresentation, 0755) != 0 && errno != EEXIST) {
        completion(rootHideError(BootstrapErrorCodeFailedExtracting, [NSString stringWithFormat:@"Failed to create secondary jbroot: %s", strerror(errno)]));
        return;
    }
    chown(secondary.fileSystemRepresentation, 0, 0);

    // The RootHide bootstrap keeps writable /var data in the AppGroup root.
    NSString *rootVar = [jbroot stringByAppendingPathComponent:@"var"];
    NSString *secondaryVar = [secondary stringByAppendingPathComponent:@"var"];
    if ([fm fileExistsAtPath:rootVar] && ![fm fileExistsAtPath:secondaryVar]) {
        if (![fm moveItemAtPath:rootVar toPath:secondaryVar error:&error]) {
            completion(error);
            return;
        }
    }
    [fm removeItemAtPath:rootVar error:nil];
    if (![fm createSymbolicLinkAtPath:rootVar withDestinationPath:secondaryVar error:&error]) {
        completion(error);
        return;
    }

    NSString *rootTmp = [jbroot stringByAppendingPathComponent:@"tmp"];
    NSString *secondaryTmp = [secondary stringByAppendingPathComponent:@"var/tmp"];
    if ([fm fileExistsAtPath:rootTmp] && ![fm fileExistsAtPath:secondaryTmp]) {
        if (![fm moveItemAtPath:rootTmp toPath:secondaryTmp error:&error]) {
            completion(error);
            return;
        }
    }
    [fm removeItemAtPath:rootTmp error:nil];
    if (![fm createSymbolicLinkAtPath:rootTmp withDestinationPath:@"var/tmp" error:&error]) {
        completion(error);
        return;
    }

    NSString *secondaryMarker = [secondary stringByAppendingPathComponent:@".jbroot"];
    [fm removeItemAtPath:secondaryMarker error:nil];
    if (![fm createSymbolicLinkAtPath:secondaryMarker withDestinationPath:jbroot error:&error]) {
        completion(error);
        return;
    }

    NSString *basebin = [jbroot stringByAppendingPathComponent:@"basebin"];
    [fm removeItemAtPath:basebin error:nil];
    error = [self extractTar:[[NSBundle.mainBundle.bundlePath stringByAppendingPathComponent:@"basebin.tar"] stringByStandardizingPath] toPath:jbroot];
    if (error) {
        completion(error);
        return;
    }
    ...
    NSString *sources = [NSString stringWithFormat:
        @"Types: deb\nURIs: https://roothide.github.io/\nSuites: ./\nComponents:\n\n"
         "Types: deb\nURIs: https://roothide.github.io/procursus\nSuites: iphoneos-arm64e/%@\nComponents: main\n\n",
        @"1900"];
    [sources writeToFile:rootHideJbrootPath(@"/etc/apt/sources.list.d/roothide.sources") atomically:YES encoding:NSUTF8StringEncoding error:nil];

    [[NSData data] writeToFile:rootHideJbrootPath(@"/.installed_dopamine") atomically:YES];
    JBFixMobilePermissions();
    completion(nil);
}
...
- (NSError *)deleteBootstrap
{
    ...
    NSString *jbroot = rootHideFindJbroot();
    if (jbroot) {
        NSString *secondary = [NSString stringWithFormat:@"/var/mobile/Containers/Shared/AppGroup/%@", jbroot.lastPathComponent];
        [NSFileManager.defaultManager removeItemAtPath:secondary error:nil];
        [NSFileManager.defaultManager removeItemAtPath:jbroot error:&error];
    }
    [NSFileManager.defaultManager removeItemAtPath:@"/var/jb" error:nil];
...
- (BOOL)isJailbroken
{
    return jbclient_roothide_jailbroken();
}

- (NSString *)jailbrokenVersion
{
    if (![self isJailbroken]) return nil;
    NSString *version = [NSString stringWithContentsOfFile:rootHideJbrootPath(@"/basebin/.version") encoding:NSUTF8StringEncoding error:nil];
    return [[version componentsSeparatedByString:@"."] lastObject];
}

- (BOOL)isBootstrapped
{
    return gSystemInfo.jailbreakInfo.rootPath != NULL;
}
...
{
    // RootHide creates the randomized root during prepareBootstrap after
    // kernel privileges have been established.
