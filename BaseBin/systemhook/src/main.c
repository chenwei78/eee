...
__attribute__((constructor)) static void initializer(void)
{
	...
	if (load_executable_path() == 0) {
		// RootHide and rootless use different system-process hook bundles.  The
		// RootHide bundle is preferred when it is present in the active jbroot.
		if (!strcmp(gExecutablePath, "/usr/sbin/cfprefsd") ||
			...
			if (roothideHooksPath && access(roothideHooksPath, F_OK) == 0) {
